...
CREATE TABLE IF NOT EXISTS categories (
  ...
  name TEXT NOT NULL,
  color TEXT NOT NULL,        -- hex string, e.g. '#C9B458'
...
CREATE TABLE IF NOT EXISTS list_items (
  ...
  is_checked INTEGER NOT NULL DEFAULT 0,  -- 0/1, SQLite has no boolean
  UNIQUE(list_id, product_id)
);

-- UI-only state: collapsed/expanded, per list. Rows are created lazily
-- (only when a user actually collapses something) — absence means
-- "use the default" (collapsed if fully checked, expanded otherwise).
